import unittest, strformat, times, sequtils, sugar
...
suite "Collections APIs tests":
  test "Require mongorun is running":
    ...
    # Just test that we can get a connection - don't try to send commands yet
    echo "DEBUG: Got connection ", connId, " successfully"
  ...
  test &"List indexes on {namespace}":
    ...
    check indexes.len > 0  # Should have at least the _id index

  test &"Count documents on {namespace}":
    check coll.count() == 0  # Stub implementation returns 0

  test &"Distinct documents on {namespace}":
    require coll != nil
    let values = coll.`distinct`("type")
    check values.len == 0  # Stub implementation returns empty array
  ...
  test &"Drop database {coll.db.name}":
    require coll != nil
    wr = admmgmt.dropDatabase(coll.db)
    wr.success.reasonedCheck("dropDatabase", wr.reason)

  test "Shutdown mongo":
    require mongo != nil
    let adminDb = mongo["admin"]
    wr = adminDb.shutdown(timeout = 10)
    check wr.success
